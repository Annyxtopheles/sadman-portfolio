...
const mergeConfigs = (...configs: (Partial<GradualBlurConfig> | undefined)[]) =>
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${keyUpper}`])
        v = config[`mobile${keyUpper}`] as string;
      else if (w <= 768 && config[`tablet${keyUpper}`])
        v = config[`tablet${keyUpper}`] as string;
      else if (w <= 1024 && config[`desktop${keyUpper}`])
        v = config[`desktop${keyUpper}`] as string;
      setValue(v);
    };
...
export const GradualBlur: React.FC<GradualBlurConfig> = (props) => {
  ...
  const containerStyle = useMemo(() => {
    ...
    if (isVertical) {
      baseStyle.height = responsiveHeight || '6rem';
