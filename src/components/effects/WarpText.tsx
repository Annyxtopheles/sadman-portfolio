...
const drawLine = (
  ...
  let cursor = align === 'left' ? 0 : align === 'right' ? totalWidth - lineWidth : x - lineWidth / 2;
...
const buildTextCanvas = ({
  ...
  const fontWeight = computed.fontWeight || String(props.fontWeight ?? 400);
  ...
  // Split lines by explicit \n, then word-wrap any line that exceeds maxWidth
  let lines: string[] = [];
  ...
  rawParagraphs.forEach(para => {
    ...
    words.forEach(word => {
      ...
      if (measureLine(ctx, test, letterSpacing) > maxWidth && currentLine) {
  ...
  if (lines.length === 0) lines = [String(props.text || '')];
  ...
  lines.forEach((line, index) =>
    drawLine(ctx, line, startX, startY + index * lineHeight, letterSpacing, align, width)
  );
...
export const WarpText: React.FC<WarpTextProps> = ({
  ...
  useEffect(() => {
    ...
    // Initialize with a 1x1 fully transparent pixel so it CANNOT render an empty/white texture
    ...
    const renderOnce = () => {
      if (disposed || contextLost || !hasRasterized) return;
  ...
  return (
    <div
      ref={containerRef}
      className={`warp-text align-${textAlign} ${className}`.trim()}
      style={style}
      role="img"
      aria-label={text}
    >
      {/* Zero-latency typographic fallback rendered cleanly while WebGL initializes */}
      <span
        ...
        style={{
          ...
          lineHeight: typeof lineHeight === 'number' ? String(lineHeight) : (lineHeight ?? '1'),
          color: color || 'inherit',
          textAlign
        }}
      >
