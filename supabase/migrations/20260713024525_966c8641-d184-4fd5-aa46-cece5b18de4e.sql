...
BEGIN
  ...
  _hash := encode(digest(_salt || _passkey, 'sha256'), 'base64');
...
BEGIN
  ...
  _hash := encode(digest(_cfg.passkey_salt || _passkey, 'sha256'), 'base64');
...
REVOKE ALL ON FUNCTION public.matrimony_unlock(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.matrimony_set_passkey(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.matrimony_unlock(text) TO anon, authenticated;